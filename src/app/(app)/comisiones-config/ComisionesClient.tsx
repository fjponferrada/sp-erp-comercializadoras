...
export default function ComisionesClient({ initialTiers, products, services }: Props) {
  ...
  const handleDuplicateTier = (tier: CommissionTier) => {
    setEditingTier({
      ...tier,
      id: '', // Actúa como nuevo
      name: `${tier.name} (Copia)`,
      copyFromId: tier.id, // Pasamos el origen para que el backend copie sus reglas
    } as any);
    setIsTierModalOpen(true);
  };
          ...
          <div key={tier.id} className="card bg-slate-900 border border-slate-800 shadow-xl overflow-hidden animate-fade-in-up">
            ...
            <div className="flex items-center justify-between p-5 bg-slate-800/50 border-b border-slate-800">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => handleEditTier(tier)} className="p-2 text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors border border-slate-700" title="Editar Nivel">
                  <Edit2 size={16} />
                </button>
                <button onClick={() => handleDuplicateTier(tier)} className="p-2 text-indigo-400 hover:text-indigo-300 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors border border-slate-700" title="Duplicar Nivel">
                  <Copy size={16} />
                </button>
