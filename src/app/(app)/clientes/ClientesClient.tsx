      ...
      <main className="px-4 md:px-8 py-6 md:py-8 flex-1 flex flex-col gap-6 w-full min-w-0">
        ...
        <div className="card animate-fade-in-up delay-400 w-full min-w-0" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--border)', display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginRight: '8px' }}>
            ...
            {showStats && (
              <div style={{ position: 'relative', minWidth: '160px' }}>
                <select
                  className="form-input w-full"
                  style={{ fontSize: '0.85rem', cursor: 'pointer', appearance: 'none', paddingRight: '36px' }}
                  value={tipoFilter}
                ...
                <SlidersHorizontal size={13} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', pointerEvents: 'none' }} />
              </div>
            )}
          </div>
