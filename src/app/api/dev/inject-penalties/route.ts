...
export async function GET() {
  ...
  for (const line of lines) {
    ...
    const [contractCode, dateStr, invoiceNumber, bipenStr, totalStr] = parts;
    const issueDate = parseDate(dateStr);
    const bipen = parseEuro(bipenStr);
    const totalAmount = parseEuro(totalStr);

    logs.push(`Processing ${invoiceNumber} for contract ${contractCode}...`);
    ...
    if (!contract) {
      logs.push(`  Contract not found: ${contractCode}`);
      continue;
    }
    ...
    if (!existing) {
      ...
      logs.push(`  -> Created PenaltyInvoice ${invoiceNumber}`);
    } else {
      logs.push(`  -> PenaltyInvoice ${invoiceNumber} already exists.`);
    }
    ...
    logs.push(`  -> Updated Contract ${contractCode}`);
    count++;
  }

  return NextResponse.json({ success: true, count, logs });
}
