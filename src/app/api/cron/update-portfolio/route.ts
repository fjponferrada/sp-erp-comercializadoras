...
export async function POST() {
  try {
    ...
    for (let i = 0; i < hoursToGenerate; i++) {
      ...
      const futurePrice = futureMap.get(m) || 65.0;
        ...
        if (p.subtype === 'CARGA_BASE') {
          ...
          if (p.profileData && Array.isArray(p.profileData)) {
            ...
            if (profile[month] && profile[month][hour] !== undefined) {
              // Convertimos asumiendo que el JSON está en kW y necesitamos pasarlo a MW (MWh/hora)
              mwThisHour = Number(profile[month][hour]) / 1000.0;
            }
          }
        }
