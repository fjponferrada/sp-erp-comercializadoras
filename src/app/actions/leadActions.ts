...
export async function createLeadAction(formData: FormData, apiContext?: { userId: string, brandId: string, allowedAutoTariffs: string[], defaultSource: string }) {
  let user: any;

  if (apiContext) {
    user = {
      id: apiContext.userId,
      brandId: apiContext.brandId,
      allowedAutoTariffs: apiContext.allowedAutoTariffs,
      defaultSource: apiContext.defaultSource
    };
  } else {
    ...
    if (!dbUser) throw new Error('Usuario no encontrado en la BD');
    user = dbUser;
  }
