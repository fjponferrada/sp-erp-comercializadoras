...
export default function LiquidacionesPpa() {
  ...
  const handleCalcular = async () => {
    ...
    // Lanzar cálculo de proyección de forma paralela
    setLoadingProjection(true);
    fetch('/api/ppas/proyeccion', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ppaId: selectedPpa })
    })
      .then(r => r.json())
      .then(d => {
        if (d.success) setProjectionResult(d.projection);
      })
  ...
  return (
    <div style={{ background: 'var(--bg-elevated)', borderRadius: '12px', border: '1px solid var(--border)', padding: '24px' }}>
      ...
      {(projectionResult || loadingProjection) && (
        <div style={{ marginTop: '32px', paddingTop: '32px', borderTop: '1px solid var(--border)' }}>
          <h4 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Calendar size={18} color="var(--accent)" />
            Proyección Financiera (Próximos 12 meses)
          </h4>
          
          {loadingProjection ? (
             <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-secondary)' }}>Calculando proyección a futuro...</div>
          ) : projectionResult ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                  <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid var(--border)' }}>
                    <th style={{ padding: '12px 16px', fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Mes</th>
                    <th style={{ padding: '12px 16px', fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', textAlign: 'right' }}>Energía Estimada (MWh)</th>
                    <th style={{ padding: '12px 16px', fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', textAlign: 'right' }}>OMIE Medio Estimado (€/MWh)</th>
                    <th style={{ padding: '12px 16px', fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', textAlign: 'right' }}>Liquidación Estimada (€)</th>
                  </tr>
                </thead>
                <tbody>
                  {projectionResult.map((row: any) => (
                    <tr key={row.monthKey} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td style={{ padding: '12px 16px', color: 'var(--text-primary)', fontWeight: 500 }}>{row.monthStr}</td>
                      <td style={{ padding: '12px 16px', color: 'var(--text-secondary)', textAlign: 'right' }}>{row.totalMwh.toLocaleString('es-ES', { maximumFractionDigits: 2 })}</td>
                      <td style={{ padding: '12px 16px', color: 'var(--text-secondary)', textAlign: 'right' }}>{row.omieMedio.toLocaleString('es-ES', { maximumFractionDigits: 2 })}</td>
                      <td style={{ padding: '12px 16px', fontWeight: 600, textAlign: 'right', color: row.isFijo ? (row.totalEur >= 0 ? '#22C55E' : '#EF4444') : '#EF4444' }}>
                        {row.totalEur.toLocaleString('es-ES', { maximumFractionDigits: 2 })} €
                      </td>
                    </tr>
                  ))}
                  <tr style={{ background: 'rgba(0,0,0,0.1)' }}>
                    <td style={{ padding: '12px 16px', color: 'var(--text-primary)', fontWeight: 700 }}>TOTAL ANUAL ESTIMADO</td>
                    <td style={{ padding: '12px 16px', color: 'var(--text-primary)', fontWeight: 700, textAlign: 'right' }}>
                      {projectionResult.reduce((sum: number, r: any) => sum + r.totalMwh, 0).toLocaleString('es-ES', { maximumFractionDigits: 2 })}
                    </td>
                    <td style={{ padding: '12px 16px', color: 'var(--text-primary)', fontWeight: 700, textAlign: 'right' }}>
                      {(projectionResult.reduce((sum: number, r: any) => sum + (r.omieMedio * r.totalMwh), 0) / projectionResult.reduce((sum: number, r: any) => sum + r.totalMwh, 0) || 0).toLocaleString('es-ES', { maximumFractionDigits: 2 })}
                    </td>
                    <td style={{ padding: '12px 16px', fontWeight: 700, textAlign: 'right', color: projectionResult[0]?.isFijo ? (projectionResult.reduce((sum: number, r: any) => sum + r.totalEur, 0) >= 0 ? '#22C55E' : '#EF4444') : '#EF4444' }}>
                      {projectionResult.reduce((sum: number, r: any) => sum + r.totalEur, 0).toLocaleString('es-ES', { maximumFractionDigits: 2 })} €
                    </td>
                  </tr>
                </tbody>
              </table>
