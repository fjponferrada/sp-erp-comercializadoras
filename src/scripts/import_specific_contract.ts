...
async function run() {
  const CUPS_TARGET = 'ES0031101326004012AL0F';
  console.log(`Iniciando importación SEGURA EXCLUSIVA para el CUPS: ${CUPS_TARGET}...`);
  ...
  console.log("Buscando el contrato en Airtable...");
  const contractsRecord = await base('CONTRATOS').select({ filterByFormula: `CUPS='${CUPS_TARGET}'` }).all();
  if (contractsRecord.length === 0) {
      console.log(`NO SE ENCONTRÓ NINGÚN CONTRATO CON EL CUPS ${CUPS_TARGET} EN AIRTABLE.`);
      return;
  }

  const contracts = contractsRecord.map(r => ({ id: r.id, fields: r.fields as any }));
  console.log(`✅ Obtenido el contrato objetivo (Airtable ID: ${contracts[0].id}). Descargando dependencias...`);
  ...
  console.log(`Dependencias descargadas. Iniciando inyección segura (vinculando sin sobrescribir)...`);
  ...
  for (const record of contracts) {
    ...
    if (!client) {
      console.log(`Creando cliente nuevo (${vatNumber})...`);
      ...
      console.log(`Vinculando a cliente existente (${vatNumber}). No se sobrescriben datos.`);
    }
    ...
    if (!supply) {
      console.log(`Creando instalación nueva (${cups})...`);
      ...
      supply = await prisma.supplyPoint.create({
        data: {
          ...
          annualConsumption: (parseFloat(sf['CONSUMO ANUAL KWH']) || parseFloat(f['CONSUMO ANUAL KWH']) || 0) / 1000,
      ...
      console.log(`Vinculando a instalación existente (${cups}). No se sobrescriben datos.`);
    }
    ...
    if (!contract) {
      ...
      contract = await prisma.contract.create({
        data: {
          ...
          p1e: f['P1E'] ? parseFloat(f['P1E']) : null,
          p2e: f['P2E'] ? parseFloat(f['P2E']) : null,
          p3e: f['P3E'] ? parseFloat(f['P3E']) : null,
          p4e: f['P4E'] ? parseFloat(f['P4E']) : null,
          p5e: f['P5E'] ? parseFloat(f['P5E']) : null,
          p6e: f['P6E'] ? parseFloat(f['P6E']) : null,
          p1p: f['P1P'] ? parseFloat(f['P1P']) : null,
          p2p: f['P2P'] ? parseFloat(f['P2P']) : null,
          p3p: f['P3P'] ? parseFloat(f['P3P']) : null,
          p4p: f['P4P'] ? parseFloat(f['P4P']) : null,
          p5p: f['P5P'] ? parseFloat(f['P5P']) : null,
          p6p: f['P6P'] ? parseFloat(f['P6P']) : null,
          p1c: f['P1C'] ? parseFloat(f['P1C']) : null,
          p2c: f['P2C'] ? parseFloat(f['P2C']) : null,
          p3c: f['P3C'] ? parseFloat(f['P3C']) : null,
          p4c: f['P4C'] ? parseFloat(f['P4C']) : null,
          p5c: f['P5C'] ? parseFloat(f['P5C']) : null,
          p6c: f['P6C'] ? parseFloat(f['P6C']) : null,
          fee: (f['Fee Index'] || f['Fee/MWh'] || f['Fee / Margen Comercial']) ? parseFloat(f['Fee Index'] || f['Fee/MWh'] || f['Fee / Margen Comercial']) : null,
          deviationCost: f['Coste Desvíos'] ? parseFloat(f['Coste Desvíos']) : null,
          cgBolsilloSolar: f['CG Bolsillo Solar'] ? parseFloat(f['CG Bolsillo Solar']) : null,
          pexc: f['Precio Excedentes'] ? parseFloat(f['Precio Excedentes']) : null,
      ...
      console.log(`✅ Contrato importado correctamente (ID: ${contract.id}).`);
    } else {
      console.log(`El contrato ya existía en el ERP. No se sobrescriben datos.`);
    }
    ...
    if (leadData || true) {
        ...
        if (!lead) {
            ...
            console.log(`✅ Lead vinculado creado.`);
        }
    }
  }

  console.log("🚀 MIGRACIÓN SEGURA FINALIZADA CON ÉXITO!");
}
