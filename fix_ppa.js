...
async function query() {
  ...
  if (res.rows.length > 0) {
      ...
      if (Array.isArray(pd) && pd.length === 12) {
          // The data is currently shifted by +1 month (0=Feb, 11=Jan)
          // We need to shift it back (0=Jan, 1=Feb)
          // So new[0] = old[11], new[1] = old[0], etc.
          const newPd = [];
          newPd[0] = pd[11];
          for(let i = 1; i < 12; i++) {
              newPd[i] = pd[i-1];
          ...
          console.log('Fixed profile data successfully.');
