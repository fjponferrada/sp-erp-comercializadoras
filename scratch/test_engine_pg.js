...
async function main() {
  ...
  const res = await client.query(`
    ...
    FROM "LoadCurve"
    WHERE cups LIKE 'ES0031105569492001JZ%'
  ...
  for (const row of res.rows) {
    ...
    // FLAWED LOGIC USED BY INTERNAL BILLING ENGINE:
    ...
    if (dayTotal > 0) {
      console.log(`LC Date Iso: ${dateIsoStr} -> Total: ${(dayTotal * 1000).toFixed(2)} kWh`);
      totalCch += dayTotal;
    }
  }

  console.log(`Flawed Engine Total CCH: ${(totalCch * 1000).toFixed(2)} kWh`);
  await client.end();
}
