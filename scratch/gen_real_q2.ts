  ...
  try {
    ...
    for (const inv of invoices) {
      ...
      if (regimenFiscalDB !== 'SBFO' && regimenFiscalDB !== 'SBFI') {
         ...
         const concepto = ''; 
         const cieDeclarante = ''; // Replaced later
    ...
    fs.writeFileSync('Z:\\AED\\AEAT\\560\\Desglose_560_Espaa_2026_T2_Abril_Mayo_REAL.txt', csvContentAM);
    fs.writeFileSync('Z:\\AED\\AEAT\\560\\Desglose_560_Espaa_2026_T2_Junio_REAL.txt', csvContentJun);

    console.log("Real TXTs generated.");
  } catch (e) {
    console.error(e);
  } finally {
    await client.end();
