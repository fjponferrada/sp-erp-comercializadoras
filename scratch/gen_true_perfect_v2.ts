...
async function run() {
  ...
  try {
    ...
    for (const inv of invoices) {
      ...
      let zona = 'España';
      if (cp.startsWith('31') || prov.includes('navarra')) zona = 'Navarra';
      else if (cp.startsWith('01') || prov.includes('alava') || prov.includes('álava')) zona = 'Álava';
      else if (cp.startsWith('20') || prov.includes('guipuzcoa') || prov.includes('guipúzcoa') || prov.includes('gipuzkoa')) zona = 'Guipúzcoa';
      else if (cp.startsWith('48') || prov.includes('vizcaya') || prov.includes('bizkaia')) zona = 'Vizcaya';
      ...
      if (minSuperadoValue !== undefined && String(minSuperadoValue).trim() !== '') {
         isMinApplied = String(minSuperadoValue).trim().toLowerCase() === '0' || String(minSuperadoValue).trim().toLowerCase() === 'false';
      } else {
      ...
      if (!regimenFiscalDB || regimenFiscalDB === 'SBFO' || regimenFiscalDB === 'SBFI') {
         ...
         if (!isNaN(cnaePrefix) && cnaePrefix >= 5 && cnaePrefix <= 33) regimenFiscalDB = 'SBFI';
         else regimenFiscalDB = 'SBFO';
      }
      ...
      if (regimenFiscalDB !== 'SBFO' && regimenFiscalDB !== 'SBFI') {
         const reduccionValue = (inv.ieDiscount && inv.ieDiscount > 0) ? round2(subtotal * (inv.ieDiscount / 100)) : 0;
    ...
    fs.writeFileSync('Z:\\AED\\AEAT\\560\\Desglose_560_Espaa_2026_T2_Abril_Mayo_PERFECT_V2.txt', csvContentAM);
    fs.writeFileSync('Z:\\AED\\AEAT\\560\\Desglose_560_Espaa_2026_T2_Junio_PERFECT_V2.txt', csvContentJun);

    console.log("Files generated without format bugs.");
  } catch (e) {
    console.error(e);
  } finally {
    await client.end();
