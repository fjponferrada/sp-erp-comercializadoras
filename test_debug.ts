...
async function main() {
  ...
  for (let h = 0; h < 24; h++) {
    ...
    if (h === 0) {
      console.log('H=0 DEBUG');
      console.log('kFactor:', kFactor[0]);
      console.log('p20, p30, p61:', p20, p30, p61);
    }
    ...
    for (const seg of Object.keys(consumption)) {
      if (consumption[seg][h]) {
        ...
        if (h === 0) {
          console.log(`seg: ${seg}, val: ${consumption[seg][h]}, underlyingTariff: ${underlyingTariff}, lossP: ${lossP}`);
        }
      }
    }
  }
