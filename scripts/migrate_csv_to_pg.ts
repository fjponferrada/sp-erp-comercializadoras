    ...
    chunk: async (results, parser) => {
      ...
      for (const row of results.data as any) {
        ...
        if (batch.length >= BATCH_SIZE) {
          try {
            const cupsList = Array.from(new Set(batch.map((b: any) => b.cups)));
            const datesList = Array.from(new Set(batch.map((b: any) => b.date)));

            // Delete existing to overwrite
            await prisma.loadCurve.deleteMany({
              where: {
                cups: { in: cupsList as string[] },
                date: { in: datesList as Date[] }
              }
            });
