...
async function run() {
  console.log("Iniciando sincronización INCREMENTAL de Airtable...");
  
  // No borramos la base de datos en una sincronización incremental
  ...
  console.log("Descargando TODOS LOS CONTRATOS de Airtable para comparar...");
  ...
  const existingDbContracts = await prisma.contract.findMany({ where: { airtableId: { not: null } }, select: { airtableId: true } });
  const existingIds = new Set(existingDbContracts.map(c => c.airtableId));
  
  const missingRecords = records.filter(r => !existingIds.has(r.id));
  console.log(`Se han encontrado ${missingRecords.length} contratos que faltan en la base de datos local y serán sincronizados.`);

  if (missingRecords.length === 0) {
    console.log("¡Todo está al día! No hay contratos nuevos que importar.");
    return;
  }
  ...
  for (const prodRec of allProducts) {
    ...
    await prisma.product.upsert({
      where: { id: prodRec.id },
      update: { name: name.toString().trim(), type: type.toString().trim(), brandId: brand.id, tariff: safeTariff, p1e, p2e, p3e, p4e, p5e, p6e, p1p, p2p, p3p, p4p, p5p, p6p, fee, feeExcedentes, pexc, permanenceMonths, commissionType, cgBolsilloSolar, hasSelfConsumption, selfConsumptionType, airtableData: p as any },
      create: { id: prodRec.id, name: name.toString().trim(), type: type.toString().trim(), brandId: brand.id, tariff: safeTariff, p1e, p2e, p3e, p4e, p5e, p6e, p1p, p2p, p3p, p4p, p5p, p6p, fee, feeExcedentes, pexc, permanenceMonths, commissionType, cgBolsilloSolar, hasSelfConsumption, selfConsumptionType, airtableData: p as any }
    });
  }
  ...
  if (failedContracts.length > 0) {
    const fs = require('fs');
    fs.writeFileSync('sync_errors.json', JSON.stringify(failedContracts, null, 2));
    console.log(`\n¡Sincronización terminada con ${failedContracts.length} errores! Revisa el archivo sync_errors.json`);
  } else {
    console.log(`\n¡Sincronización finalizada con éxito total (0 errores)!`);
  }
}
